...
import { Plus, Trash2, Search, ArrowDownCircle, ArrowUpCircle, Clock } from 'lucide-react';
...
export default function DecaissementsPage() {
  ...
  const [formRetrait, setFormRetrait] = useState({
    description: '', dateOperation: new Date().toISOString().split('T')[0],
    montantTotal: '', repartitions: {} as Record<string, string>, notes: '',
  });
  ...
  const sauvegarder = async (type: 'ajout'|'retrait') => {
    ...
    if (type === 'ajout') setFormAjout(reset);
    else setFormRetrait(reset);
    setSaving(false);
    charger();
  };
  ...
  const filtered = tousMovements.filter((d: any) =>
  ...
  const Formulaire = ({ type }: { type: 'ajout'|'retrait' }) => {
    const form    = type === 'ajout' ? formAjout : formRetrait;
    const setForm = type === 'ajout' ? setFormAjout : setFormRetrait;
    const totalRep = Object.values(form.repartitions).reduce((s, v) => s + (parseInt(v) || 0), 0);
    const color    = type === 'ajout' ? 'green' : 'red';
      ...
      <div className={clsx(
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="date" value={form.dateOperation}
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className={clsx('rounded-2xl p-4 text-center border',
          ...
          <p className={clsx('text-xs font-medium',
            soldeNet >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400')}>
            Solde net
          </p>
          <p className={clsx('text-lg font-bold mt-1',
            soldeNet >= 0 ? 'text-blue-700 dark:text-blue-400' : 'text-red-700 dark:text-red-400')}>
            {formatFCFA(soldeNet)}
          </p>
        </div>
      </div>
      ...
      <div className="bg-[var(--surface)] rounded-2xl border border-[var(--border)] p-4 transition-colors">
        <h3 className="font-semibold text-[var(--text)] mb-3 text-sm">Solde par fond</h3>
            ...
            return (
              <div key={c.id} className="bg-slate-50 dark:bg-dark-card rounded-xl p-3 text-center">
                ...
                <p className={clsx('text-sm font-bold mt-1',
                  solde >= 0 ? 'text-primary' : 'text-red-500')}>
                  {formatFCFA(solde)}
                </p>
      ...
      {/* ── Onglet Ajout ── */}
      {onglet === 'ajout' && <Formulaire type="ajout" />}

      {/* ── Onglet Retrait ── */}
      {onglet === 'retrait' && <Formulaire type="retrait" />}
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {d.repartitions?.filter((r: any) => Number(r.montant) > 0).map((r: any) => (
                              <span key={r.id}
                                className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs rounded-full px-2 py-0.5">
                ...
                <button key={p} onClick={() => setPage(p)}
                  className={clsx('w-8 h-8 rounded-lg text-sm font-medium transition-all',
                    p === page
                      ...
                      : 'border border-[var(--border)] text-[var(--text-muted)] hover:bg-slate-50 dark:hover:bg-dark-card')}>
                  {p}
                </button>
              ))}
              <button
