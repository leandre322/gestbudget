...
import { Trash2, Search, ArrowDownCircle, ArrowUpCircle, Lock } from 'lucide-react';
...
function Formulaire({ type, form, setForm, comptes, banques, saving, isLocked, onSave }: FormulaireProps) {
  ...
  return (
    <div className={clsx(
      ...
      {/* Bannière lock */}
      {isLocked && (
        <div className="flex items-center gap-2 bg-slate-50 dark:bg-dark-card border border-[var(--border)] rounded-xl px-4 py-2.5">
          <Lock size={13} className="text-slate-400 flex-shrink-0"/>
          <p className="text-sm text-[var(--text-muted)]">Page verrouillée — cliquez "Modifier" dans la bannière pour enregistrer.</p>
        </div>
      )}
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="sm:col-span-2">
          ...
          <input type="text" value={form.description} disabled={isLocked}
        ...
        <div>
          ...
          <input type="date" value={form.dateOperation} disabled={isLocked}
      ...
      <div>
        ...
        <input type="number" value={form.montantTotal} disabled={isLocked}
          onChange={e => setForm(f => ({ ...f, montantTotal: e.target.value }))}
          placeholder="0"
          className={clsx('w-48 border rounded-xl px-3 py-2 text-sm outline-none',
            isLocked ? 'border-[var(--border)] bg-slate-50 dark:bg-dark-card text-[var(--text-muted)] cursor-not-allowed'
                     : 'border-[var(--border)] bg-[var(--card)] text-[var(--text)] focus:border-primary')}/>
      </div>
      ...
      <div>
        ...
        {comptes.length > 0 && (
          <div className="mb-3">
            <p className="text-xs text-[var(--text-muted)] font-medium mb-2 uppercase tracking-wide">Fonds de fonctionnement</p>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-2.5">
              {comptes.map((c: any) => (
                <div key={c.id} className="flex items-center gap-2">
                  <span className="text-sm text-[var(--text)] flex-1 truncate">{c.nom}</span>
                  <input type="number" value={form.repartitions[c.id] ?? ''} disabled={isLocked}
                    onChange={e => setForm(f => ({ ...f, repartitions: { ...f.repartitions, [c.id]: e.target.value } }))}
                    placeholder="0"
                    className={clsx('w-28 border rounded-lg px-2 py-1.5 text-sm text-right outline-none',
                      isLocked ? 'border-[var(--border)] bg-slate-50 dark:bg-dark-card text-[var(--text-muted)] cursor-not-allowed'
                               : 'border-[var(--border)] bg-[var(--card)] text-[var(--text)] focus:border-primary')}/>
                </div>
              ))}
        ...
        {banques.length > 0 && (
          <div>
            <p className="text-xs text-[var(--text-muted)] font-medium mb-2 uppercase tracking-wide">Comptes bancaires</p>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-2.5">
              {banques.map((b: any) => (
                <div key={b.id} className="flex items-center gap-2">
                  <span className="text-sm text-[var(--text)] flex-1 truncate">{b.nomBanque}</span>
                  <input type="number" value={form.repartitions[`banque_${b.id}`] ?? ''} disabled={isLocked}
                    onChange={e => setForm(f => ({ ...f, repartitions: { ...f.repartitions, [`banque_${b.id}`]: e.target.value } }))}
                    placeholder="0"
                    className={clsx('w-28 border rounded-lg px-2 py-1.5 text-sm text-right outline-none',
                      isLocked ? 'border-[var(--border)] bg-slate-50 dark:bg-dark-card text-[var(--text-muted)] cursor-not-allowed'
                               : 'border-[var(--border)] bg-[var(--card)] text-[var(--text)] focus:border-primary')}/>
      ...
      <div>
        ...
        <input type="text" value={form.notes} disabled={isLocked}
          onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
          placeholder="Remarques..."
          className={inputCls}/>
      </div>

      <button onClick={onSave} disabled={saving || isLocked || !form.description || !form.montantTotal}
        title={isLocked ? 'Verrouillez pour modifier' : undefined}
        className={clsx('px-5 py-2 rounded-xl text-sm font-medium text-white transition-all disabled:opacity-50',
          type === 'ajout' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-500 hover:bg-red-600')}>
        {isLocked ? <span className="flex items-center gap-1.5"><Lock size={13}/>Verrouillé</span>
         : saving ? 'Enregistrement...'
         : type === 'ajout' ? 'Valider l\'ajout' : 'Valider le retrait'}
      </button>
    </div>
  );
}
...
export default function DecaissementsPage() {
  ...
  const charger = useCallback(async () => {
    ...
    if (resB.ok) { const db = await resB.json(); setBanques(db.banques ?? []); }
    setLoading(false);
  }, [annee]);
  ...
  const sauvegarder = async (type: 'ajout' | 'retrait') => {
    ...
    for (const { banqueId, montant } of Object.values(repartitionsBanques)) {
      await fetch(`/api/banques?id=${banqueId}`, {
        method:  'PUT',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ action: type === 'ajout' ? 'increment' : 'decrement', montant }),
      });
    }
    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className={clsx('rounded-2xl p-4 text-center border',
          soldeNet >= 0 ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800')}>
          <p className={clsx('text-xs font-medium', soldeNet >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400')}>Solde net</p>
          <p className={clsx('text-lg font-bold mt-1', soldeNet >= 0 ? 'text-blue-700 dark:text-blue-400' : 'text-red-700 dark:text-red-400')}>{formatFCFA(soldeNet)}</p>
        </div>
      </div>

      <div className="flex gap-1 bg-slate-100 dark:bg-dark-card rounded-xl p-1 w-fit border border-[var(--border)]">
        {([['ajout','Ajout','text-green-600'],['retrait','Retrait','text-red-500'],['historique','Historique','text-blue-600']] as const).map(([key,label,color]) => (
          <button key={key} onClick={() => setOnglet(key)}
            className={clsx('px-4 py-2 rounded-lg text-sm font-medium transition-all',
              onglet === key ? 'bg-[var(--surface)] shadow-sm ' + color : 'text-[var(--text-muted)] hover:text-[var(--text)]')}>
            {key === 'ajout' ? '➕' : key === 'retrait' ? '➖' : '📋'} {label}
          </button>
        ))}
      </div>

      {onglet === 'ajout' && (
        <Formulaire type="ajout" form={formAjout} setForm={setFormAjout} comptes={comptes} banques={banques}
          saving={saving} isLocked={isLocked} onSave={() => sauvegarder('ajout')}/>
      )}
      {onglet === 'retrait' && (
        <Formulaire type="retrait" form={formRetrait} setForm={setFormRetrait} comptes={comptes} banques={banques}
          saving={saving} isLocked={isLocked} onSave={() => sauvegarder('retrait')}/>
      )}
      ...
      {onglet === 'historique' && (
        <div className="space-y-4">
          ...
          <p className="text-xs text-[var(--text-muted)]">{filtered.length} opération(s) — Page {page} / {Math.max(1, totalPages)}</p>
                  ...
                  {paginated.length === 0 ? (
                    ...
                    return (
                      <tr key={d.id} className="border-t border-[var(--border)] hover:bg-slate-50/60 dark:hover:bg-dark-card/60 transition-colors">
                        ...
                        <td className={clsx('px-4 py-3 text-right font-semibold', isAjout ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400')}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <button onClick={() => supprimer(d.id)}
                            disabled={isLocked} title={isLocked ? 'Verrouillez pour modifier' : undefined}
                            className={clsx('transition-colors', isLocked ? 'text-slate-200 dark:text-slate-700 cursor-not-allowed' : 'text-slate-300 hover:text-red-500')}>
                            <Trash2 size={14}/>
                          </button>
                        </td>
                      </tr>
