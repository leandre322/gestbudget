...
import { revalidateTag } from 'next/cache' // ← S5 : invalidation cache analytiques
...
export async function PUT(req: NextRequest) {
  try {
      ...
      await prisma.budgetMensuel.upsert({
        where: {
          userId_anneeId_categorieId_mois: {
            userId: data.userId, anneeId: data.anneeId,
            categorieId: data.categorieId, mois: data.mois,
          },
        },
    ...
    await logAudit({
      ...
      entityType: 'budget', details: { mois, anneeId }, req,
    })

    // S5 : invalide le cache analytiques — les graphiques refléteront les nouvelles données
...
export async function POST(req: NextRequest) {
  try {
    ...
    const csrfErrPost = csrfCheck(req)
    if (csrfErrPost) return csrfErrPost
    ...
    await logAudit({
      userId: session.user.id, action: 'update',
      entityType: 'budget', entityId: ligne.id,
      details: { mois, anneeId }, req,
    })
